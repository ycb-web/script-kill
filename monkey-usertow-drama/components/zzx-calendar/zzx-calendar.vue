<template>
	<view class="zzx-calendar">
		<view class="calendar-heander flex w100 justSb bs-bb p20">
			<div class="left flex w50 justS ali-C">
			<image @tap="daysPre()" src="../../static/icon33.png" mode=""></image><view>{{timeStr}}</view><image @tap="daysNext()" src="../../static/icon34.png" mode="">
			</div>
			<div class="right flex ali-C" @click="navTo(`/pagesA/my/memberActiveList`)">
				<text class="c-fff mr-2">会员码</text><image src="../../static/icon35.png" mode=""></image>
			</div>
		</view>
		...
		<view class="calendar-content">
		   <swiper class="calendar-swiper" :style="{
			   width: '100%',
			   height: sheight
		   }" :indicator-dots="false" :autoplay="false" :duration="duration" :current="current" @change="changeSwp" :circular="true" :disable-touch="true">
					...
					<view class="calendar-days">
						<template v-if="sitem === current">
							<view class="calendar-day" v-for="(item,index) in days" :key="index" :class="{ 'day-hidden': !item.show }" @click="clickItem(item)">
								<div class="inner m_auto " :class="[ item.fullDate === selectedDate ? checkedClass : '',itemshijian(item.time)==sameday?'act' : '' ]">
									
								<!-- <div class="inner m_auto" :class="[ item.isToday ? todayClass : '', item.fullDate === selectedDate ? checkedClass : '']"> -->
									<view class="date">
										
									{{item.time.getDate()}}
									</view>
									<view style="font-size: 24rpx;line-height: 30rpx;">活动</view>
								</div>
							</view>
						</template>
						<template v-else>
			...
			<view class="mode-change" @click="changeMode">
				<image v-if="!weekMode" src="../../static/icon37.png" mode=""></image>
				<image v-if="weekMode" src="../../static/icon38.png" mode=""></image>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		computed: {
			itemshijian(){
				return (date) =>{
					console.log(date,"----------")
					let shijian = new Date(date)
					let nian = shijian.getFullYear()
					let yue = shijian.getMonth()
...
.zzx-calendar {
	...
	height: auto;
	padding-top: 10rpx;
	background-image: linear-gradient(#07C17B, #038E45);
	.calendar-heander {
		text-align: center;
		height: 60upx;
		line-height: 60upx;
	...
	.is-today {
		background: url(../../static/icon36.png) ;
		background-size: 100% 100%;
		color: #ffffff;
		border-radius: 10rpx;
	}
	.is-checked {
		background: url(https://shop666.oss-cn-hangzhou.aliyuncs.com/front/HuoDongRiLi.png) no-repeat;
